...
class constrain_check:
      ...
      def metadata_checks(self,baseline={},parameters=[]):
          ...
          if not type(baseline) == dict   or not type(parameters) == list:
             print("Value Error :- baseline should be a Dictionary and parameters should be a List ")
          ...
          if not missing_list__.__len__() == 0:
             print (colored("Metadata Problem Library Dependencies Missing",'red',attrs=['reverse', 'blink']))
             print(colored("Missing Baseline Dictionary Variables are : " + str(missing_list__), attrs=['reverse', 'blink']))
             self.print_error_info(inspect.stack()[2][1].split("/")[-1].split(".")[0],inspect.stack()[2][1])
             sys.exit(1)
      ...
      def check_network(self,address,port):
           try:
                ...
                if not test_conn == 0:
                    print(colored("Check Network Settings/Service Status ;  Connectivity issues " + address + ":" + port + " not reachable", attrs=['reverse', 'blink']))
                    self.print_error_info(inspect.stack()[2][1].split("/")[-1].split(".")[0],inspect.stack()[2][1])
                    sys.exit(1)
      ...
      def check_file_exists(self,file_path):
          ...
          if (not os.path.exists(file_path)):
             print(colored("Specified File/File Path " + file_path + " does not exist", 'magenta'))
             self.print_error_info(inspect.stack()[2][1].split("/")[-1].split(".")[0], inspect.stack()[2][1])
             sys.exit(1)
      ...
      def check_file_type(self,file_path,file_type):
          ...
          if (not file_format == file_type):
              print(colored("Cannot Parse File Content " + file_path + " contains : " + file_format,'magenta'))
              sys.exit()
      ...
      def mongo_collection_creation_constrain(self,data):
          ...
          if not type(data) == list or not type(data[0]) == dict:
              print(colored("Data Format Errors for MongoDb Collection Creation Kindly check the data being inserted", 'magenta'))
              self.print_error_info(inspect.stack()[2][1].split("/")[-1].split(".")[0], inspect.stack()[2][1])
              sys.exit(1)
      ...
      def check_command_exists(self,command=[]):
          ...
          if not missing_commands.__len__() == 0:
             print("Commands missing for safe execution of program :- ")
      ...
      def print_error_info(self,class_name,file_name):
          print()
          print(colored("Calling Object/Class Name :- " + class_name, attrs=['reverse', 'blink']))
          print(colored("Class File location : " + file_name, attrs=['reverse', 'blink']))
          print()
